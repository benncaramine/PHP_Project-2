...
<style>
        ...
        $result=$bdd->query("SELECT * FROM `approvisionnement`  ");
        echo "<table class=","content-table",">";
        echo   "<thead>";
        echo   "<tr>";
        echo "<th>id</th>";
        echo "<th>productId</th>";
        echo "<th>fournisseurID</th>";
        echo "<th>quantite</th>";
        echo "<th>date</th>";
        echo "<th></th>";
        ...
        while ($row = $result->fetch(PDO::FETCH_NUM)) {     
            echo    "<tr>";
            echo "<td>".$row[4]."</td> ";
            $x=$bdd->query("SELECT libelle FROM produit where ref = ".$row[0].";");
            $q=$x->fetch(PDO::FETCH_NUM);
            echo "<td>".$q[0]."</td> ";
            $x=$bdd->query("SELECT nom FROM fournisseur where id = ".$row[1].";");
            $q=$x->fetch(PDO::FETCH_NUM);
            echo "<td>".$q[0]."</td> ";
            echo "<td>".$row[2]."</td> ";
            echo "<td>".$row[3]."</td> ";

            echo "<td><a href=","deleteAppro.php?ref=".$row[4].""," id=","delete",">delete</a></td>";
            echo    "</tr>";
...
?>
    <a href="insertApprovisionnement.php" id="ajouter">ADD</a>
            </center>
        </body>
</html>